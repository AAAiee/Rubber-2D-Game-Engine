...
namespace Rubber{
	...
	void Timer::startFrame()
	{
		...
		if (fpsUpdateElapsedTime.count() >= Timer::FPS_UPDATE_INTERVAL){
	...
	void Timer::waitForFrameEnd()
	{
		...
		this->m_FrameTimeIndex = (this->m_FrameTimeIndex + 1) % Timer::MAX_FRAME_HISTORY;
		this->m_FrameTimeCount = min(m_FrameTimeCount + 1, Timer::MAX_FRAME_HISTORY);
	}
