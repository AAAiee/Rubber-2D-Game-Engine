...
namespace Rubber {
	...
	Scope<Rubber::Shader> Shader::create(std::string_view filepath)
	{
	...
	Scope<Rubber::Shader> Shader::create(std::string_view name, std::string_view vertexSrc, std::string_view fragmentSrc)
	{
		switch (Renderer::getAPI()) {
		case RendererAPI::API::NONE:   RB_CORE_ASSERT(false, "NONE-API type is not supported");
    ...
    const Ref<Rubber::Shader> ShaderLib::getShader(std::string_view name) const 
    {  
