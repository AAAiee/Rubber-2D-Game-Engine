...
#define RB_PROFILE_ENABLED 0
	...
	#define RB_PROFILE_SCOPE(name) ::Rubber::AutoProfile profile##__LINE__(name)
	#define RB_PROFILE_FUNC() RB_PROFILE_SCOPE(__FUNCSIG__) 
	#define RB_PROFILE_BEGIN_SESSION(name,filepath) \
					::Rubber::Instrumentor::getInstance().beginSession(name,filepath)
	#define RB_PROFILE_END_SESSION() \
					::Rubber::Instrumentor::getInstance().endSession()
#else
